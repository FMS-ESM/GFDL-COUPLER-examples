...
! option to override various surface boundary conditions for SCM
  ...
  use constants_mod, only: xmissing                ! bjt
...
! ------------------------------------------------
!
subroutine flux_exchange_end ( Atm, Land, Ice, Ocean, atmos_ice_boundary, land_ice_atmos_boundary, &
       land_ice_boundary, ice_ocean_boundary, ocean_ice_boundary )
  type(atmos_data_type),             intent(inout)  :: Atm
  type(land_data_type),              intent(in)  :: Land
  type(ice_data_type),               intent(inout)  :: Ice
...
!!$  else
     ...
     call put_to_xgrid (Atm%gust,   'ATM', ex_gust, xmap_sfc, complete=.false.)              ! bjt
!!$  endif
